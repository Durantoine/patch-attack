...
class TransferEvaluator:
    ...
    def _build_frame(
        ...
        panel_attacked = self._draw_detections(
            disp_attacked, _rescale(dets_attacked), COLOR_ATTACKED
        )
        for panel, label in [
            (panel_clean, f"Clean ({len(dets_clean)} pers.)"),
            (panel_attacked, f"Attaque ({len(dets_attacked)} pers.)"),
        ]:
            cv2.putText(
                panel, label, (5, size - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200, 200, 200), 1
            )

        patch_panel = patch_to_img(self.patch, size)
        cv2.putText(
            patch_panel, "Patch", (5, size - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1
        )
        return np.hstack(
            [panel_clean, panel_attacked, patch_panel, self._summary_panel(metrics, size)]
        )
    ...
    def plot_analysis(self, metrics: TransferMetrics) -> None:
        ...
        ax1.fill_between(
            x, metrics.det_clean_history, metrics.det_attacked_history, alpha=0.15, color="#2196f3"
        )
        ax1.set_ylabel("Détections YOLO (person)")
        ax1.set_title(
            f"Transférabilité DINOv3 → YOLO | {Path(VIZ_DATASET).name} "
    ...
    def report(self, metrics: TransferMetrics) -> None:
        ...
        print()
        print(
            f"Taux disparition    : {metrics.disappeared}/{metrics.images_with_person}"
